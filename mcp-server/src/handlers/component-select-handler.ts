...
export async function handleComponentSelectTask(payload: unknown): Promise<unknown> {
	debugLog('[DEBUG] component-select handler called');
	...
	try {
		...
		debugLog('[DEBUG] component-select search returned: ' + (Array.isArray(rawResults) ? rawResults.length : 'not-array'));
	}
	catch (error: unknown) {
		debugLog('[DEBUG] component-select search failed: ' + String(error));
